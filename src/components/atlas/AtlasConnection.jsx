/*
 * AtlasConnection.jsx — SVG connection between atlas tree nodes.
 *
 * Straight lines for cross-orbit/cross-group connections.
 * Circular arcs for same-orbit connections (following the orbit circle).
 * Allocated connections glow golden. Idle connections are subtle but visible.
 */

import { memo } from 'react';
import { CONNECTION_COLORS } from '../../data/atlas/atlasTreeConstants';

function AtlasConnection({ x1, y1, x2, y2, arc, bothAllocated, oneAllocated }) {
  let style;
  if (bothAllocated) {
    style = CONNECTION_COLORS.allocated;
  } else if (oneAllocated) {
    style = CONNECTION_COLORS.partial;
  } else {
    style = CONNECTION_COLORS.idle;
  }

  // Same-orbit connections: render as circular arc
  if (arc && arc.r > 0) {
    // Determine sweep direction for the minor arc (shorter path around the circle)
    const angle1 = Math.atan2(y1 - arc.cy, x1 - arc.cx);
    const angle2 = Math.atan2(y2 - arc.cy, x2 - arc.cx);
    // Clockwise angular difference
    let delta = ((angle2 - angle1) % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI);
    // sweep=1 means clockwise in SVG (positive-y-down coordinate system)
    // Use the shorter arc: if clockwise delta <= π, go clockwise; otherwise go counterclockwise
    const sweepFlag = delta <= Math.PI ? 1 : 0;

    return (
      <path
        d={`M ${x1} ${y1} A ${arc.r} ${arc.r} 0 0 ${sweepFlag} ${x2} ${y2}`}
        fill="none"
        stroke={style.stroke}
        strokeWidth={style.strokeWidth}
        strokeLinecap="round"
        opacity={style.opacity}
      />
    );
  }

  // Cross-orbit or cross-group: straight line
  return (
    <line
      x1={x1}
      y1={y1}
      x2={x2}
      y2={y2}
      stroke={style.stroke}
      strokeWidth={style.strokeWidth}
      strokeLinecap="round"
      opacity={style.opacity}
    />
  );
}

export default memo(AtlasConnection);
