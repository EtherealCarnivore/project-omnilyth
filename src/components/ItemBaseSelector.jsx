      ...
      <div ref={wrapperRef} className="relative">
        ...
        {value && (
          <div className="mt-2 flex items-center gap-2 text-xs">
            <span className="px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-300 border border-indigo-400/20">
              {value.baseType}
            </span>
        ...
        {open && filtered.length > 0 && (
          <div className="absolute z-50 mt-1 w-full max-h-64 overflow-y-auto rounded-lg bg-zinc-900 border border-white/10 shadow-xl">
            {filtered.map((base) => (
              <button
                key={`${base.baseType}-${base.item}`}
                onClick={() => handleSelect(base)}
                className={`w-full text-left px-3 py-2 text-sm hover:bg-indigo-500/10 transition-colors flex items-center justify-between ${
