import { generateNumberRegex } from './mapModRegex';

function itemNumberRegex(number) {
  return generateNumberRegex(number, false).replaceAll(".", "\\d");
}

export function generateMagicItemRegex(settings) {
  ...
  const openPrefix = (item) => `^${item}`;
  const openSuffix = (item) => `${item}$`;
...
export function generateRareItemRegex(affixMap, settings) {
  const itemBase = settings.itembase;
  const selectedMods = settings.selectedRareMods;

  if (!itemBase) return "";

  const mods = Object.entries(selectedMods)
  ...
  const result = mods
    ...
    .map((e) => {
      ...
      const regex = hasRangeInsideRegex
        ? e.regex.regex.replace(
            "\\d+",
            itemNumberRegex(e.value.values[rangeInRegex])
          )
        : e.regex.regex;
      const numbersBefore = e.regex.before
        .map((number) => e.value.values[number])
        .filter((v) => v !== undefined && v !== "")
        .map((f) => itemNumberRegex(f))
        .join(".*");
